...
async function guardAny(ctx, permissions) {
  ...
  await ctx.answerCallbackQuery({ text: 'You do not have permission for this action.', show_alert: true });
  return false;
}

// Main callback router for admin: callbacks
async function routeAdminCallback(ctx) {
  const data = ctx.callbackQuery.data;
  if (!data.startsWith('admin:')) return false;

  const action = data.slice(6); // after 'admin:'
...
// Router for all other admin-prefixed callbacks (afolder:, acfolder:, acitem:, etc.)
async function routeAdminSubCallback(ctx) {
  const data = ctx.callbackQuery.data;
  const uid = ctx.from.id;

  // Folder management: afolder:
  if (data.startsWith('afolder:')) {
    if (!await guard(ctx, PERMISSIONS.MANAGE_FOLDERS)) return true;
    const parts = data.slice(8).split(':');
    const sub = parts[0];
    if (sub === 'create') {
      const parentId = parts[1] || null;
      await folderHandlers.startCreate(ctx, parentId);
    } else if (sub === 'rename') {
      await folderHandlers.startRename(ctx, parts[1]);
    } else if (sub === 'desc') {
      await folderHandlers.startDesc(ctx, parts[1]);
    } else if (sub === 'toggle') {
      await folderHandlers.toggleActive(ctx, parts[1]);
    } else if (sub === 'delete') {
      await folderHandlers.deleteFolder(ctx, parts[1]);
    } else {
      await folderHandlers.showFolderDetail(ctx, parts[0]);
    }
    return true;
  }

  // Content management: acfolder:, acitem:, acupload:
  if (data.startsWith('acfolder:')) {
    if (!await guardAny(ctx, [PERMISSIONS.UPLOAD_CONTENT, PERMISSIONS.EDIT_CONTENT, PERMISSIONS.DELETE_CONTENT])) return true;
    const folderId = data.slice(9);
    await contentHandlers.showFolderContent(ctx, folderId);
    return true;
  }
  if (data.startsWith('acitem:')) {
    if (!await guardAny(ctx, [PERMISSIONS.UPLOAD_CONTENT, PERMISSIONS.EDIT_CONTENT, PERMISSIONS.DELETE_CONTENT])) return true;
    const contentId = data.slice(7);
  ...
  if (data.startsWith('acedit_')) {
    ...
    if (parts[0] === 'title') {
      await contentHandlers.startEditTitle(ctx, parts[1]);
    } else if (parts[0] === 'desc') {
      await contentHandlers.startEditDesc(ctx, parts[1]);
    }
    return true;
  }
  ...
  // Force join type selection (step 3 of add flow): afj_type:
  if (data.startsWith('afj_type:')) {
    if (!await guard(ctx, PERMISSIONS.MANAGE_FORCE_JOIN)) return true;
    const type = data.slice(9);
    await forceJoinHandlers.handleTypeSelection(ctx, type);
    return true;
  }
  ...
  if (data.startsWith('afj:')) {
    ...
    const sub = parts[0];
    if (sub === 'add') {
      await forceJoinHandlers.startAdd(ctx);
    } else if (sub === 'edit_name') {
      await forceJoinHandlers.startEditName(ctx, parts[1]);
    } else if (sub === 'edit_link') {
      await forceJoinHandlers.startEditLink(ctx, parts[1]);
    } else if (sub === 'edit_user') {
      await forceJoinHandlers.startEditUsername(ctx, parts[1]);
    } else if (sub === 'edit_id') {
      await forceJoinHandlers.startEditId(ctx, parts[1]);
    } else if (sub === 'toggle') {
      await forceJoinHandlers.toggleChannel(ctx, parts[1]);
    } else if (sub === 'toggle_approval') {
      await forceJoinHandlers.toggleApproval(ctx, parts[1]);
    } else if (sub === 'delete') {
      await forceJoinHandlers.deleteChannel(ctx, parts[1]);
    } else {
      await forceJoinHandlers.showChannelDetail(ctx, parts[0]);
    }
    return true;
  }
  ...
  if (data.startsWith('awelcome:')) {
    ...
    if (sub === 'preview') {
      await welcomeHandlers.preview(ctx);
    } else {
      await welcomeHandlers.startEdit(ctx, sub);
    }
    return true;
  }
  ...
  if (data.startsWith('abc:')) {
    ...
    if (sub === 'text') {
      await broadcastHandlers.startText(ctx);
    } else if (sub === 'photo') {
      await broadcastHandlers.startPhoto(ctx);
    } else if (sub === 'video') {
      await broadcastHandlers.startVideo(ctx);
    } else if (sub === 'history') {
      await broadcastHandlers.showHistory(ctx);
    }
    return true;
  }
  ...
  if (data.startsWith('auser:')) {
    ...
    if (parts[0] === 'search') {
      await userHandlers.startSearch(ctx, false);
    } else if (parts[0] === 'search_user') {
      await userHandlers.startSearch(ctx, true);
    } else if (parts[0] === 'block') {
      await userHandlers.toggleBlock(ctx, Number(parts[1]));
    }
    return true;
  }

  // Admin management: aadmin:
  if (data.startsWith('aadmin:')) {
    if (!await guard(ctx, PERMISSIONS.MANAGE_ADMINS)) return true;
    const parts = data.slice(7).split(':');
    const sub = parts[0];
    if (sub === 'add') {
      await adminMgmtHandlers.startAdd(ctx);
    } else if (sub === 'remove') {
      await adminMgmtHandlers.removeAdmin(ctx, Number(parts[1]));
    } else if (sub === 'perms') {
      await adminMgmtHandlers.showPermissionsMenu(ctx, Number(parts[1]));
    } else if (sub === 'toggle_perm') {
      await adminMgmtHandlers.togglePermission(ctx, Number(parts[1]), parts[2]);
    } else {
      await adminMgmtHandlers.showAdminDetail(ctx, Number(parts[0]));
    }
    return true;
  }
  ...
  if (data.startsWith('astorage:')) {
    ...
    const sub = data.slice(9);
    if (sub === 'set') {
      await storageHandlers.startSet(ctx);
    } else if (sub === 'test') {
      await storageHandlers.testConnection(ctx);
    }
    return true;
  }
  ...
  if (data.startsWith('asettings:')) {
    ...
    if (sub === 'language') {
      await settingsHandlers.startLanguage(ctx);
    } else if (sub === 'autodelete') {
      await settingsHandlers.startAutoDelete(ctx)
    } else if (sub === 'referrals') {
      await settingsHandlers.startReferrals(ctx);
    } else if (sub === 'referrals_off') {
      await settingsHandlers.disableReferrals(ctx);
    } else if (sub === 'referral_timezone') {
      await settingsHandlers.startReferralTimezone(ctx);
    } else if (sub === 'view') {
      await settingsHandlers.viewAll(ctx);
    }
    return true;
  }
...
// Handle text input during admin flows — dispatches to the right handler
